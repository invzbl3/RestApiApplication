...
(
    order_id      varchar        not null auto_increment,
...
alter table orders
    add constraint FK_order_passport foreign key (passport_id)
...
alter table orders
    add constraint FK_order_vehicle foreign key (vehicle_id)
        references vehicles (vehicle_id) on delete restrict on update restrict;

alter table users
    add constraint FK_usertype_user foreign key (usertype_id)
